...
var sql = {
	...
	area_list:{'username':'用户名','password':'密码','name':'昵称','email':'邮箱'},
	...
	edit_info:["UPDATE user SET ","=? WHERE id=?"],
...
router.post('/login', function(req, res, next) {
  ...
  connection.query(sql.login,[username,password],function(err,rows,fields){
  ...
  connection.query(sql.username_verification,[req.body.username],function(err,rows,fields){
	  ...
	  else
	  {
		  connection.query(sql.name_verification,[req.body.name],function(err,rows,fields){
			...
			{
				connection.query(sql.register,[req.body.username,password,req.body.name,req.body.email],function(err,rows,fields){
					if(!err)
					{
						req.session.user = {
...
router.post('/userInfo',function(req,res,next){
	...
	{
		var connection = require("../database/db");
		connection.query(sql.edit_info[0]+req.body.field+sql.edit_info[1],[req.body.data,req.session.user.id],function(err,rows,fields){
			if(err)
			{
				res.json({
					status:1,
...
router.post('/character',function(req,res,next){
	...
	var connection = require("../database/db");
	connection.query(sql.add_character,[req.body.name,duty,req.body.race,req.body.profession,req.session.user.id],function(err,rows,fields){
		if(err)
		{
			res.json({
				status:1,
...
router.delete('/character',function(req,res,next){
	var connection = require("../database/db");
	connection.query(sql.delete_character,[req.body.id,req.session.user.id],function(err,rows,fields){
		if(err)
		{
			res.json({
				status:1,
