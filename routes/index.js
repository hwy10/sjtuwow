...
router.get('/index/editNotice/:target/:title', function(req, res, next) {
  res.render('notice', {target:req.params.target,pageTitle:req.params.title});
});
...
router.post('/index/notice', function(req, res, next) {
  var connection = require("../database/db");
  connection.query(sql.postNotice,[req.body.content,req.session.user.id,req.body.target],function(err,rows,fields){
	if(err)
	{
		res.json({
			status:1,
